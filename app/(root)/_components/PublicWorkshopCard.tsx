...
const AdminWorkshopCard = ({data, isEnrolled}:WorkshopCardProps) => {
    ...
    return (
        <Card className={`group relative py-1 gap-0 ` + (isDeleted ?"bg-red-500/20" :"")}>
            ...
            <CardContent className="p-4">
                ...
                {isEnrolled ?
                    <Link href={`/dashboard/workshops/${data.slug}`} className={buttonVariants({className: "w-full mt-4", variant: "blue"})}>
                        Follow Workshop
                    </Link>
                    :
                    <Link href={`/workshops/${data.slug}`} className={buttonVariants({className: "w-full mt-4"})}>Learn
                        More</Link>
                }
            </CardContent>
        </Card>
