...
import {addObjectiveToCourse, addRequisiteToCourse} from "@/app/admin/courses/[courseId]/edit/actions";

const RequisitesObjectivesForm = ({courseId, type}: {
    courseId: string,
    ...
    const onSubmit = (values: ObjectiveRequisiteSchema) => {
        startTransition(async () => {
            if (type === 'Requisite') {
                await addRequisiteToCourse(courseId, values)
            } else {
                await addObjectiveToCourse(courseId, values)
            }
            form.reset({content: ""});
            setOpen(false)
