...
import RequisitesObjectivesForm from "@/app/admin/learning-paths/[id]/edit/_components/RequistesObjectivesForm";
import DeleteBtnItemObjReq from "@/app/admin/learning-paths/[id]/edit/_components/DeleteBtnItemObjReq";

const LearningPathSettings = ({id, tags, requisites, objectives}:
                              {
                                  id: string,
            ...
            <Card className="my-6">
                ...
                <CardContent className="grid grid-cols-1 gap-4 lg:grid-cols-2">
                    <div className=" space-y-3 bg-muted-foreground/10 p-4 rounded-md border flex  flex-col gap-4">
                            ...
                            {requisites && requisites.length > 0 ?
                                requisites.map((item, index) => (
                                   ...
                                   <span className="flex gap-2 items-center">
                                        ...
                                        <DeleteBtnItemObjReq key={ index} learningPathId={id} type={"Requisite"} item={item} />
                                    </p>
                                ))
                                :
                        ...
                        <RequisitesObjectivesForm learningPathId={id} type={"Requisite"}/>
                    </div>
                    <div className=" space-y-3 bg-muted-foreground/10 p-4 rounded-md border flex  flex-col gap-4">
                        <h2>Learning Path Objectives :</h2>
                        <div className="flex-1  space-y-3">
                            {objectives && objectives.length > 0 ?
                                objectives.map((item, index) => (
                                   ...
                                   <span className="flex gap-2 items-center">
                                        ...
                                        <DeleteBtnItemObjReq key={ index} learningPathId={id} type={"Objective"} item={item} />
                                    </p>
                                ))
                                :
                                <p className="text-muted-foreground text-base">
                        ...
                        <RequisitesObjectivesForm learningPathId={id} type={" Objective"}/>
                ...
                <CardContent>
                    <div className=" space-y-3">
                        ...
                        <Suspense fallback={<AdminTagCardSkeletonLayout/>}>
                            <RenderTags learningPathId={id} tags={tags.map(tag => ({...tag.tag}))}/>
                        </Suspense>
                    </div>
                </CardContent>
            </Card>
