...
export async function updateStateSubscriptionPlan(planId: string, state:string): Promise<ApiResponseType> {
    ...
    try {
        ...
        return {
            status: "success",
            message: "Subscription plan  state update successfully"
        }
    } catch (e) {
        return {
            status: "error",
            message: "Failed to update  subscription plan state"
        }
    }
