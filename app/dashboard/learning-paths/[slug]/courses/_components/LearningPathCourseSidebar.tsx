...
import {
    ...
    LearningPathLessonLinkItem
} from "@/app/dashboard/learning-paths/[slug]/courses/_components/LearningPathLessonItem";


export function LearningPathCourseSidebar({course, enrolled, learningPathSlug}: {course : CourseType, enrolled: boolean, learningPathSlug: string}) {
                      ...
                      <CollapsibleContent className={"pt-3 pl-6  space-y-3 "}>
                          {chapter.lessons.map((lesson) => {
                              ...
                              if(hasAccess(lesson.public, course, enrolled))
                                  return  <LearningPathLessonLinkItem lesson={lesson}
                                                      slug={course.slug}
                                                      key={lesson.id}
                              ...
                              return <LearningPathLessonItem lesson={lesson}
                                          slug={course.slug}
                                          key={lesson.id}
                                          isActive={currentLessonId === lesson.id}
                                          completed={lesson.lessonProgress.find((progress) =>
